...
1. On **SEA-ADM1**, oduble click on the **Azure portal** shortcut.

   ![Picture 1](media/azure-portal.png)
...
3. In the Azure portal, in the **Search resources, services, and docs** text box, on the toolbar, search for and select **Microsoft Defender for Cloud**.
   ...
   ![Picture 1](media/defenderforcloud.png)

4. On the **Microsoft Defender for Cloud \| Getting started** page, select **Upgrade**, and then select **Install agents (1)** tab, and select **Install agents (2)**.

   ![Picture 1](media/install-agents.png)
...
1. On **SEA-ADM1**, in the Azure portal, in the **Search resources, services, and docs** text box, on the toolbar, search **Virtual machines (1)** and select **Virtual machines (2)**. In the **Virtual machines** page, select **+ Create**, and then select **Azure virtual machine**.

   ![Picture 1](media/virtualmachines.png)
...
4. Select **Next: Advanced >**, on the **Advanced** tab of the **Create a virtual machine** page, review the available settings without modifying any of them, and then select **Review + Create**.
   ...
   >**Note**: Do not create the virtual machine; use the autogenerated template for this purpose.
...
1. Once the file **template.zip** downloaded, then in the pop-up menu, select **Show in folder**. This will automatically open File Explorer displaying the content of the **Downloads** folder.

1. Now, extract the **template.zip** folder to the **C:\Labfiles\AZ-800-Administering-Windows-Server-Hybrid-Core-Infrastructure-master\Allfiles\Labfiles\Lab06** folder on **SEA-ADM1** (create a new folder if needed).

1. Navigate back to the **Azure Portal**. From the **Template** page, browse back to the **Create a virtual machine** page, and close it without completing the deployment.
...
3. On the **Settings & monitoring** page, in the list of extensions, to the right side of the **Log Analytics agent/Azure Monitor agent** entry, select the **Edit configuration** link.
   ...
   ![Picture 1](media/configuration.png)
...
1. In the Azure portal, browse back to the **AZ800** page, and then in the list of resources, select the entry representing the network interface, select **IP configurations**, and then select the **ipconfig1** entry.
...
1. Browse back to the **az800l06-vm0** page, select **Connect**, and select **Connect**.

On **az800l06-vm0 \| Connect** page, scroll down and click **Select** option in **Native RDP** under **Most Common** section.
